...
from radare2_mcp.models.schemas import SearchResult, ROPGadget, Address
...
class TestSearchTools:
    ...
    async def test_search_assembly(self, test_session):
        ...
        results = await SearchTools.search_assembly(
            assembly="mov eax",
            session_id="test_session"
        )
    ...
    async def test_search_magic(self, test_session):
        ...
        results = await SearchTools.search_magic(
            session_id="test_session"
        )
    ...
    async def test_search_references(self, test_session):
        ...
        results = await SearchTools.search_references(
            address=addr,
            session_id="test_session"
        )
    ...
    async def test_get_all_strings(self, test_session):
        ...
        results = await SearchTools.get_all_strings(
            min_length=4,
            session_id="test_session"
        )
    ...
    async def test_search_pattern_with_wildcards(self, test_session):
        ...
        results = await SearchTools.search_pattern(
            pattern="7f45??46",  # ELF with wildcard
            session_id="test_session"
        )
